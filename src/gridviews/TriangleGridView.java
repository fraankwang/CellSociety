...
public class TriangleGridView extends GridView {
     ...
     */
    private Double[] generateTriangleCoordinates (Double x, Double y, int cellSize, boolean pointedUp) {
    	...
    	for (int row = 0; row < getMyRows(); row++) {
            for (int col = 0; col < getMyColumns(); col++) {
                ...
                if (row % 2 == 0) {
                	if (col % 2 == 0) {
                		shape.getPoints().addAll(generateTriangleCoordinates(
                				xOffset, yPointDownOffset, cellSize, false));
                	}
                	else {
                		shape.getPoints().addAll(generateTriangleCoordinates(
                				xOffset, yPointUpOffset, cellSize, true));
                	}
